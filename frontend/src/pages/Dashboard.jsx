...
import ChatBox from '../components/chat/ChatBox'; // IMPORT CHAT BOX
  ...
  const renderCitizenCards = () => (
    <>
      <div className="group bg-white border border-slate-200 rounded-3xl p-8 hover:shadow-lg hover:border-indigo-200 transition-all duration-300 flex flex-col justify-between">
        <div>
          <div className="w-14 h-14 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center mb-6 text-slate-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300 shadow-sm">
        ...
        <button onClick={() => navigate('/create-request')} className="w-full bg-slate-900 hover:bg-indigo-600 text-white py-3.5 rounded-xl font-medium transition-all duration-300 shadow-sm">
          Create Request
        </button>
      </div>
      ...
      <div className="group bg-white border border-slate-200 rounded-3xl p-8 hover:shadow-lg hover:border-indigo-200 transition-all duration-300 flex flex-col justify-between">
        ...
        <button onClick={() => navigate('/organizations')} className="w-full bg-white border-2 border-slate-200 text-slate-700 hover:bg-indigo-50 hover:border-indigo-600 hover:text-indigo-700 py-3 rounded-xl font-semibold transition-all duration-300 shadow-sm">
          View Organizations
        </button>
      </div>
    </>
  ...
  const renderVolunteerCards = () => (
    <>
      <div className="group bg-white border border-slate-200 rounded-3xl p-8 hover:shadow-lg hover:border-emerald-200 transition-all duration-300 flex flex-col justify-between">
        <div>
          <div className="w-14 h-14 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center mb-6 text-slate-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors duration-300 shadow-sm">
        ...
        <button onClick={() => navigate('/tasks')} className="w-full bg-slate-900 hover:bg-emerald-600 text-white py-3.5 rounded-xl font-medium transition-all duration-300 shadow-sm">
          Open Task Board
        </button>
      </div>
      ...
      <div className="group bg-white border border-slate-200 rounded-3xl p-8 hover:shadow-lg hover:border-emerald-200 transition-all duration-300 flex flex-col justify-between">
        ...
        <button onClick={() => navigate('/my-tasks')} className="w-full bg-white border-2 border-slate-200 text-slate-700 hover:bg-emerald-50 hover:border-emerald-600 hover:text-emerald-700 py-3 rounded-xl font-semibold transition-all duration-300 shadow-sm">
          View My Tasks
        </button>
      </div>
    </>
        ...
        <div className="relative overflow-hidden bg-slate-900 rounded-3xl p-8 sm:p-10 shadow-lg border border-slate-800">
          <div className="relative z-10">
            ...
            <p className="text-slate-400 text-sm md:text-base max-w-xl leading-relaxed">
              {user?.role === 'volunteer' ? 'Thank you for stepping up to help your community.' : user?.role === 'organization' ? 'Manage your community campaigns from your command center.' : 'Manage your community requests and track assistance from your central command center.'}
            </p>
            <div className="flex items-center gap-4 mt-6">
              <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border ${user?.role === 'volunteer' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : user?.role === 'organization' ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' : 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30'}`}>
                {user?.role}
              </span>
              <span className="text-slate-400 text-sm font-medium">{user?.email}</span>
            </div>
          </div>
          <div className={`absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 rounded-full mix-blend-multiply filter blur-3xl opacity-20 pointer-events-none ${user?.role === 'volunteer' ? 'bg-emerald-600' : user?.role === 'organization' ? 'bg-amber-500' : 'bg-indigo-600'}`}></div>
        </div>
                ...
                return (
                  <div key={item.id} className="group border border-slate-200 rounded-2xl p-6 hover:border-indigo-300 transition-colors duration-300 bg-white flex flex-col lg:flex-row justify-between lg:items-center gap-6">
                    ...
                    <div className="flex flex-col items-start lg:items-end border-t lg:border-t-0 pt-4 lg:pt-0 gap-3 min-w-[150px]">
                      ...
                      {/* CHAT BUTTON (Shows if task is accepted) */}
